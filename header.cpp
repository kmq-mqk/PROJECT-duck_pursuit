...
void GameStart(){
    ...
    bool init = false;
    Score score(10000, 0.1, 1); // 10,000 điểm, mỗi 0.1s trừ 1
        ...
        switch (currentScreen){
            ...
            case GAMEPLAY:
            {
                ...
                if (!init) {
                    ...
                    init = true;
                    score.Reset(); // reset điểm
                    score.Start(); // bắt đầu countdown
                }
                ...
                score.Update();
                Render(alterVec, cellSize, 0.5, score);
            ...
            case ENDING:
            {
                BeginDrawing();
                    ...
                    score.Draw((SCREEN_WIDTH - MeasureText(TextFormat("SCORE: %05i", score.Value()), 48)) / 2, SCREEN_HEIGHT / 2, 48, BLACK);
                EndDrawing();
                init = false;
            }break;
            default: break;
